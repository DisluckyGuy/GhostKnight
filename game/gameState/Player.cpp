...
void Player::initHealthBar()
{
    ...
    healthBarRect.setPosition(ge::data.win.mapPixelToCoords(sf::Vector2i(41 + posXOffset,healthRectY)));
    ...
    shieldBarRect.setPosition(ge::data.win.mapPixelToCoords(sf::Vector2i(41 + posXOffset,shieldRectY)));
...
void Player::updatePlayer()
{
    ...
    if (health <= 0) {
        alive = false;
        player.setTextureRect(sf::IntRect(141,0,50,35));
...
void Player::updateHealthBar()
{
    ...
    healthBarRect.setPosition(ge::data.win.mapPixelToCoords(sf::Vector2i(41 + posXOffset,healthRectY), ge::data.win.getView()));
    shieldBarRect.setPosition(ge::data.win.mapPixelToCoords(sf::Vector2i(41 + posXOffset,shieldRectY), ge::data.win.getView()));
    float posX = 51;
    for (sf::RectangleShape& rect : healthVec) {
        rect.setPosition(ge::data.win.mapPixelToCoords(sf::Vector2i(posX + posXOffset,healthRectY)));
        
        if (isFullScreen) {
            rect.setSize(sf::Vector2f(190 / maxHealth, 30));
            posX += 190 / maxHealth;
        } else {
            rect.setSize(sf::Vector2f(140 / maxHealth, 30));
            posX += 140 / maxHealth;
        }
    }
    posX = 51;
    for (sf::RectangleShape& rect : shieldVec) {
        rect.setPosition(ge::data.win.mapPixelToCoords(sf::Vector2i(posX + posXOffset,shieldRectY)));
        if (isFullScreen) {
            rect.setSize(sf::Vector2f(190 / maxShield, 30));
            posX += 190 / maxShield;
        } else {
            rect.setSize(sf::Vector2f(140 / maxShield, 30));
            posX += 140 / maxShield;
        }
    }
}
